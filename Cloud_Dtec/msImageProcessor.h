...
public:
 ...
 /*\/\/\/\/\/\/\/\/\/\/*/
  ...
  RegionList *GetBoundariesAndRegions( void );
  ...
  /* 以下为自定义的函数                                                                     */
  /************************************************************************/
  int* GetLabels();			// 获取多边形的标识
  int* GetMPC();			// 获取多边形的面积，也就是每一个多边形所包含的像素的个数
  float* GetModeData();		// 获取各个多边形的特征数据
  byte * ModeData2RGB();	// 获取每个多边形的灰度值
  int* GetRegionPtIdxs();	// 用一个数组记录每个多边形所包含的像素的索引
...
private:
  ...
  //========================
   ...
   /***********************************************华丽的分割线**************************************************/
	...
	void DefineRegions( void );			// defines the boundaries of each region using the classified segmented
